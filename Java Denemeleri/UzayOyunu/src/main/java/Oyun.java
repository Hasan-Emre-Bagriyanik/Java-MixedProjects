...
public class Oyun extends JPanel implements KeyListener,ActionListener{
    ...
    public boolean kontrolEt(){
        for(Ates ates: atesler){
            if(new Rectangle(ates.getX(),ates.getY(),15,30).intersects(new Rectangle(topX,0,30,30))){
                return true;
            }
        }
        return false;
    ...
    public Oyun() {
        ...
        try {
            image = ImageIO.read(new FileImageInputStream(new File("uzaygemisi.jpg")));
    ...
    public void paint(Graphics g) {
        ...
        for(Ates ates : atesler){
            g.fillRect(ates.getX(), ates.getY(), 15, 30);
        }
